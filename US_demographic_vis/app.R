...
ui <- fluidPage(
  ...
  mainPanel( 
      tabPanel("Population Flux",
               ...
               choices = c("Total Pop. in 2017" = "totalpop.17",
...
server <- function(input, output) {
  ## need to insert an if-else statement thingy here
  output$all_states <- renderLeaflet({
    ...
    pal <- colorBin("YlOrRd", domain = all_us$info_1)
    ...
    leaflet(all_us) %>%
      ...
      addPolygons(
        ...
        labelOptions = labelOptions(
          ...
          textsize = "15px",
          direction = "auto")) %>%
      addLegend(pal = pal, values = ~all_us$info_1, opacity = 0.7, title = NULL,
